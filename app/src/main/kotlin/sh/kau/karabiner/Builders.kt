...
class ManipulatorBuilder {
  ...
  private fun addAllToEventList(
    eventList: MutableList<To>,
    commands: List<To>
  ): ManipulatorBuilder = apply {
  ...
  fun toIfAlone(commands: List<To>): ManipulatorBuilder =
    addAllToEventList(this.toIfAlone, commands)

  fun toIfAlone(vararg commands: To): ManipulatorBuilder =
    addAllToEventList(this.toIfAlone, commands.toList())
  ...
  fun toAfterKeyUp(commands: List<To>): ManipulatorBuilder =
    addAllToEventList(this.toAfterKeyUp, commands)

  fun toAfterKeyUp(vararg commands: To): ManipulatorBuilder =
    addAllToEventList(this.toAfterKeyUp, commands.toList())
  ...
  fun toAfterKeyUp(keyCode: KeyCode, vararg modifiers: ModifiersKeys): ManipulatorBuilder {
    val modifiersList = if (modifiers.isEmpty()) null else modifiers.toList()
    return toAfterKeyUp(keyCode, modifiers = modifiersList)
  }

  // --- toIfHeldDown ---
  fun toIfHeldDown(command: To): ManipulatorBuilder = addToEventList(this.toIfHeldDown, command)
  fun toIfHeldDown(commands: List<To>): ManipulatorBuilder =
    addAllToEventList(this.toIfHeldDown, commands)

  fun toIfHeldDown(vararg commands: To): ManipulatorBuilder {

    return addAllToEventList(this.toIfHeldDown, commands.toList())
  }
...
private fun applyAppTargetCondition(builder: ManipulatorBuilder, appTarget: String?) {
  ...
  when (appTarget.lowercase()) {
    "terminal" -> builder.withCondition(
      forApp(
        "^com\\.apple\\.Terminal$",
...
class KeyMapBuilder(private val description: String) {
  ...
  private var currentConditions =
    mutableListOf<Condition>() // Conditions active for subsequent remaps

  inner class RemapToBuilder(
    ...
    fun to(targetKey: KeyCode, targetModifiers: List<ModifiersKeys>? = null): RemapFinalizer {
      val baseManipulatorBuilder = manipulator()
        .from(
          fromKey,
          mandatoryModifiers = fromModifiers
        ) // Assuming direct remap modifiers are mandatory
      ...
      return RemapFinalizer(
        lastAddedIndex,
        provisionalManipulator,
        fromKey,
        fromModifiers,
        targetKey,
        targetModifiers
  ...
  inner class RemapFinalizer(
    ...
    fun otherwise(heldKey: KeyCode, heldModifiers: List<ModifiersKeys>? = null): KeyMapBuilder {
      val dualRoleManipulator = manipulator()
        ...
        .build()
      manipulators[manipulatorIndex] =
        applyConditionsToExisting(dualRoleManipulator, originalToManipulator.conditions)
    ...
    fun whenAlone(): KeyMapBuilder {
      ...
      val originalToTarget = originalToManipulator.to?.firstOrNull()?.keyCode
        ?: throw IllegalStateException("Original .to() target key not found for whenAlone logic")
      ...
      val dualRoleManipulator = manipulator()
        ...
        .toIfAlone(
          toKeyIfAlone,
          modifiers = toModifiersIfAlone
        ) // This is the alone action specified by the initial .to()
        .build()
      manipulators[manipulatorIndex] =
        applyConditionsToExisting(dualRoleManipulator, originalToManipulator.conditions)
      return this@KeyMapBuilder
  ...
  private fun applyConditionsToExisting(
    manipulator: Manipulator,
    conditions: List<Condition>?
  ): Manipulator {
    return if (conditions.isNullOrEmpty()) manipulator else manipulator.copy(
      conditions = (manipulator.conditions)?.plus(
        conditions
      )
    )
  }
